...
public class IntServerGUI2 extends JFrame {

	private int port;
	private ServerSocket serverSocket=null;
	private JTextArea t_display;

	public IntServerGUI2(int port) {

		super("IntServer GUI2");
	...
	private void receiveMessages(Socket cSocket) {
		...
		try {
			...
			//클라이언트 소켓으로부터 얻은 바이트 스트림에 + DataInputStream 연결.
			//DataInputStream 스트림으로 확장. + BufferedInputStream에 연결하여 버퍼 추가.
			int message; //int 형으로 바이트를 받을 예정.(DataInputStream의 readInt: 바이트 -> 정수로 변환하여 읽음)
			
			while (true) {
			
				message= ((DataInputStream)in).readInt();
			...
			//read()와 달리 -1 반환하지 않음. 
			//대신에, 클라이언트측 소켓 종료시 IOExeption 발생 -> 이를통해 연결 종료를 처리하자.
	...
	public static void main(String[] args) {
		IntServerGUI2 server = new IntServerGUI2(54321);
