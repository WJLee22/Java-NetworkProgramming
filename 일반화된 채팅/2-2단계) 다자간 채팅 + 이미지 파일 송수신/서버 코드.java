...
public class WithChatServer extends JFrame {
	...
	private void startServer() {
		...
		try {
			...
			while (acceptThread == Thread.currentThread()) {// 현재 acceptThread값이 null값이거나, 다른 스레드가 생성되어서 현재 스레드가 더 이상 이
															// while문 처리할 필요가 없음.
...
//	}
	...
	private class ClientHandler extends Thread {
		...
		private ObjectOutputStream out; // 이 ClientHandler작업 스레드마다 특정 클라이언트에 별도로 출력하도록, ClientHandler 내부로 out 스트림객체
										// 이동시킴.
		...
		private void receiveMessages(Socket cSocket) {
			...
			try {
				ObjectInputStream in = new ObjectInputStream(new BufferedInputStream(cSocket.getInputStream())); // in 은
																													// 이
																													// 메서드
																													// 내에서만
																													// 활용하니깐.
				...
				while ((msg = (ChatMsg) in.readObject()) != null) { // 입력스트림으로부터 채팅 메시지 객체를 읽어옴. 메시지의 모드값에 따라서 서로 다른
																	// 동작처리.
...
//					broadcating(message); //현재 서버에 접속한 모든 클라이언트에게 메시지 전송.
		...
		private void sendMesssage(String msg) { // 만약에 어떤 서버가 특정클라이언트에게만 전달할 필요가있다면 각 클라이언트 헨들러가갖고있는 이 sendMessage()로,
												// 전달받은 문자열 메시지를 하나의 채팅메시지 객체로 구성해서 전달하도록.
